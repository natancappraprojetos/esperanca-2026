...
export default function ChurchStepES({ church, campaign, onContinue, data }: ChurchStepESProps) {
        ...
        <div className="container-narrow flex flex-col gap-6 py-10">
          ...
          >
            ...
            <p className="text-gray-600 mb-8">
              Aún no tenemos una iglesia registrada exactamente en este barrio para esta campaña. Pero no te preocupes, ¡aún puedes descargar el material digital!
            </p>
            <button
              onClick={onContinue}
  ...
  async function handleSave() {
    ...
    if (isMobile && navigator.share) {
      try {
        ...
        await navigator.share({ files: [file], title: `Invitación — ${church!.name}` })
        setSaved(true)
        return
      } catch { /* fall through */ }
    }
    ...
    if (isMobile && navigator.share) {
      try {
        await navigator.share({
          title: `${church!.name} — Semana de la Esperanza 2026`,
          text: `¡Ven a participar en la Semana de la Esperanza! Yo estaré en ${church!.name}.`,
          url,
        })
      ...
      <div className="container-narrow flex flex-col gap-6 py-10">
        ...
        >
          <span className="text-overline" style={{ color: 'var(--green)' }}>
            ✓ Encontramos una programación cerca de ti
          </span>
        </div>
                ...
                >
                  {saved ? (
                    ...
                    <>
                      ...
                      <span>Descarga tu invitación</span>
                    </>
                  )}
                </button>
              )}
